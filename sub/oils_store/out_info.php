...
$result = mysqli_query($con, "SELECT  * FROM oil_store_out  WHERE id = '$id' ");
if(mysqli_num_rows($result) > 0 ) $row = mysqli_fetch_array($result);

$order_num = $row['order_num'];

$sum = mysqli_query($con, "SELECT SUM(price * balance) As sum,  count(*) as total  FROM oil_store_out  WHERE  is_delete = 0  AND order_num = '$order_num' ");
...
$result2 = mysqli_query($con, "SELECT  * FROM oil_store_out  WHERE  is_delete = 0  AND order_num = '$order_num' ");
  ...
  <thead>
	...
	while($row2 = mysqli_fetch_array($result2))
	{
		...
		if($type=='pre_equipments') {$done="where done = '0'";} else {$done="where 1";}
            $result = mysqli_query($con, "SELECT * FROM `" . $type . "` $done AND  id = '$direction'  AND is_delete = '0' ");
