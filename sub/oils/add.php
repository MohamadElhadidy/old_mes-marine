...
if (isset($_POST['add_button']))
{
  ...
  $lmit = strip_tags($_POST['lmit']);
	...
	if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM oils WHERE code='$code'  ")) > 0) array_push($error_array, "herecode") ;
	if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM oils WHERE  name = '$name' ")) > 0) array_push($error_array, "herename") ;
			
	if(empty($error_array))
	{
		mysqli_query($con , "INSERT INTO oils(code, name, type, balance, unit, price, lmit, notes) VALUES ('$code', '$name', '$type', '$balance', '$unit', '$price', '$lmit', '$notes')");
		$_SESSION['code'] = "";
		$_SESSION['name'] = "";
		$_SESSION['balance']= "";
    $_SESSION['unit'] = "";
...
<head>
  <meta charset="utf-8">
  <title>إضافة صنف - مخزن   الزُّيُوت و الْوَقُود</title>
...
<div class="center">
  <h1>إضافة صنف - مخزن  الزُّيُوت و الْوَقُود </h1>
  <form method='post'>
    <div class="inputbox">
