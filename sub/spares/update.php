...
//declaring variables
$result = mysqli_query($con, "SELECT * FROM spares WHERE id ='$id'");
if(mysqli_num_rows($result) > 0 ) $row = mysqli_fetch_array($result);
$type = $row['type'];

$resultType = mysqli_query($con, "SELECT * FROM spare_types WHERE id ='$type'");
if(mysqli_num_rows($resultType) > 0 ) $rowType = mysqli_fetch_array($resultType);


$error_array = array();

$find1 = "SELECT * FROM spare_store_enter where name = '$id' ";
if(mysqli_num_rows(mysqli_query($con, $find1)) > 0 ) $exist = 1;

$find2 = " SELECT * FROM spare_store_out where name = '$id' ";
...
if (isset($_POST['edit_button']))
{
  ...
  $type = strip_tags($_POST['type']);
    ...
    $notes = strip_tags($_POST['notes']);
	  ...
	  if($code != $row['code']) if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM spares WHERE code='$code'  ")) > 0) array_push($error_array, "herecode") ;

	  if($name != $row['name'])if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM spares WHERE  name = '$name' ")) > 0) array_push($error_array, "herename") ;

	if(empty($error_array))
	{
		$query =mysqli_query($con, "UPDATE spares
		SET
...
<head>
  <meta charset="utf-8">
  <title>تعديل صنف - مخزن  قطع الغيار</title>
