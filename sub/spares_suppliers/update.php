...
//declaring variables
$result = mysqli_query($con, "SELECT * FROM spare_suppliers WHERE id ='$id'");
...
if (isset($_POST['edit_button']))
{
    ...
    $notes = strip_tags($_POST['notes']);
	  ...
	  if($code != $row['code']) if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM spare_suppliers WHERE code='$code'  ")) > 0) array_push($error_array, "herecode") ;

	  if($name != $row['name'])if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM spare_suppliers WHERE  name = '$name' ")) > 0) array_push($error_array, "herename") ;

	if(empty($error_array))
	{
		$query =mysqli_query($con, "UPDATE spare_suppliers SET
...
<head>
  <meta charset="utf-8">
  <title>تعديل مورد - مخزن  قطع الغيار</title>
...
<div class="center">
  <h1>تعديل مورد - مخزن  قطع الغيار</h1>
  <form method='post'>
    <div class="inputbox">
