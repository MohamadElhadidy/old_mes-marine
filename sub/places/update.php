...
if (isset($_POST['edit_button']))
{
    ...
    $notes = strip_tags($_POST['notes']);
	  ...
	  if($code != $row['code']) if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM places WHERE code='$code'  ")) > 0) array_push($error_array, "herecode") ;

	  if($name != $row['name'])if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM places WHERE  name = '$name' ")) > 0) array_push($error_array, "herename") ;

	if(empty($error_array))
	{
		$query =mysqli_query($con, "UPDATE places
		SET
		code = '$code',name = '$name',location = '$location',size = '$size', notes = '$notes'
		WHERE id = '$id'
		");
  ...
  <form method='post'>
    <div class="inputbox">
      ...
      <br>
              ...
              <?php if(in_array("herecode", $error_array)) echo "<h3 style='font-weight:bold;color:red;'>كود المنشأة موجود بالفعل<br></h3>"; ?>

    <div class="inputbox">
      <input type="text" required="required" name="name"  value="<?php if(isset($row['name']))echo $row['name'];?>">
      <span> إسم المنشأة</span>
    </div>
    <?php if(in_array("herename", $error_array)) echo "<h3 style='font-weight:bold;color:red;'> اسم المنشأة موجود بالفعل <br></h3>"; ?>
    <div class="inputbox">
      <input type="text" required="required"  name="location"  value="<?php if(isset($row['location']))echo $row['location'];?>">
      <span> الموقع </span>
    </div> 
    <div class="inputbox">
      <input type="text" required="required"  name="size"  value="<?php if(isset($row['size']))echo $row['size'];?>">
      <span> المساحة </span>
    </div>
