...
require "../../config.php";
 ...
 header('Location: ../../');
	...
	$query1 = mysqli_query($con,"SELECT * FROM spare_store_out WHERE id ='".$_GET['id']."'");
    ...
    if(mysqli_num_rows($query) > 0 ) $row = mysqli_fetch_array($query);
        ...
        if($today >= $expire){
            ...
            mysqli_query($con,"update spares set balance = '$newBalance' where id = '".$row1['name']."'");
            mysqli_query($con,"update spare_store_out set is_delete = 1 where id = '".$_GET['id']."'");
