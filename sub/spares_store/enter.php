...
if(isset($_POST['enter_button'])){
	...
	$s = "insert into spare_store_enter (order_num, order_date, type, name, balance, price, supplier, employee, notes, date) values";
	...
	}else
	{
		for($i=0;$i<$_POST['numbers'];$i++)
		{
	     	mysqli_query($con,"update spares set balance = '$newBalance[$i]', price = '".$_POST['price'][$i]."' where id = '".$_POST['name'][$i]."'");
		}
	       	echo "<script>
    ...
    <header>
      ...
      <div class='set'>
        ...
        <option selected disabled VALUE=""> اسم الصنف</option>
			    <?php
			    $query = "SELECT * FROM spares where type = '$type' ";
