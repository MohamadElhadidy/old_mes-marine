...
$sel = "SELECT count(*) as allcount FROM spare_store_out   WHERE spare_store_out.is_delete = 0 AND tb= 'safety'   group by order_num ";
...
$sel = "SELECT count(*) as allcount FROM spare_store_out  WHERE      1  ".$searchQuery." AND spare_store_out.is_delete = 0  AND tb= 'safety' group by order_num ";
$Records = mysqli_query($con, $sel);
...
$empQuery = "SELECT * FROM spare_store_out  WHERE   1  ".$searchQuery." AND spare_store_out.is_delete = 0 AND tb= 'safety' group by order_num order by  order_num desc ".$columnName." ".$columnSortOrder."  ".$rowLength."" ; 
...
elseif($row["tb"] == "pre_equipments") $row["type"] = 'تصنيع معدات';
   ...
   $data[] = array( 
      ...
      "type"=>$row['type'],
      "show"=>'<a href="safety_info.php?id='.$row["id"].'"class="btn btn-success">عرض</a>',
